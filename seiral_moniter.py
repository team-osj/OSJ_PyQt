...
class SerialController(QWidget):
    ...
    def __init__(self):
        ...
        self.cb_commend = QComboBox()
        self.cb_commend_value = QLineEdit()
        self.cb_commend_send = QPushButton(self.tr("send"))
        self.commend_label = QLabel()
    ...
    def init_widget(self):
        ...
        grid_box.addWidget(QLabel(self.tr("Commend")), 6, 0)
        grid_box.addWidget(self.cb_commend, 6, 1)
        grid_box.addWidget(self.cb_commend_value, 6, 2)
        grid_box.addWidget(self.cb_commend_send, 6, 3)
        grid_box.addWidget(self.commend_label, 7, 2)

        self.Changcb("AT+RFSET")

        self.cb_commend_send.clicked.connect(lambda: self.SendData(self.cb_commend.currentText() + self.cb_commend_value.text()))
        self.cb_commend.activated.connect(lambda: self.Changcb(self.cb_commend.currentText()))
    ...
    def _fill_serial_info(self):
        ...
        commend_name = {0: "AT+RFSET", 1: "AT+SENSDATA_START", 2: "AT+SENSDATA_END", 3: "AT+RFSEND", 4: "AT+UPDATE", 5: "AT+SETNUM", 6: "AT+NOWSTATE",
                        7: "AT+SETDEL_WSH", 8: "AT+SETDEL_DRY"}
        self.cb_commend.insertItems(0, [commend_name[x] for x in [0,1,2,3,4,5,6,7,8]])
    ...
    def SendData(self, data):
        ...
        test = bytes([0x02]) + bytes("TEST DATA", "utf-8") + bytes([0x03])
        self.serial.writeData(data_bytes)
        print(data_bytes)

    def Changcb(self, commend):
        print(commend)
        commend_dict = {'AT+RFSET': '"XXXXX"', 'AT+SENSDATA_START': '', 'AT+SENSDATA_END': '', 'AT+RFSEND': '"원하는 문자열(영어)"',
                        'AT+SETNUM': '"X,X"', 'AT+NOWSTATE': '', 'AT+SETDEL_WSH': '"X"',
                        'AT+SETDEL_DRY': '"X"', 'AT+UPDATE': '"X,X"'}
        commend_label_dict = {'AT+RFSET': '5자리 RF 주소 설정', 'AT+SENSDATA_START': '센서 데이터 UART로 바로 전송',
                              'AT+SENSDATA_END': '센서 데이터 UART로 바로 전송 종료',
                              'AT+RFSEND': 'UART로 받은 값 RF통신으로 전송','AT+SETNUM': '채널별 장치 번호 설정',
                              'AT+NOWSTATE': '현재 RF 주소 및 채널별 장치 번호 출력',
                              'AT+SETDEL_WSH': '센서 반응 지연 시간 설정(10초간격)','AT+SETDEL_DRY': '센서 반응 지연 시간 설정(1초간격)',
                              'AT+UPDATE': '꺼짐 켜짐 RF통신으로 수동 전송(0/1,장치 번호)'}
        self.cb_commend_value.setText(commend_dict[commend])
        self.commend_label.setText(commend_label_dict[commend])
...
class Form(QWidget):
    ...
    def __init__(self):
        ...
        #self.pb_send = QPushButton("Send_a")
